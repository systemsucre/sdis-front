...
function ReportesFormularioRed() {
    const auth = useAuth()
    ...
    const [alcanceEst, setAlcalceEst] = useState([{ id: 3, nombre: 'RED' }, { id: 2, nombre: 'MUNICIPIO' }, { id: 1, nombre: 'ESTABLECIMIENTO' }]);
    const [alcanceMunicipio, setAlcalceMunicipio] = useState([{ id: 3, nombre: 'RED' }, { id: 2, nombre: 'MUNICIPIO' }]);
    const [alcance, setAlcalce] = useState([{ id: 3, nombre: 'RED' }, { id: 2, nombre: 'MUNICIPIO' }, { id: 1, nombre: 'ESTABLECIMIENTO' }]);
    ...
    try {
        ...
        const listarTodosGruposMun = async () => {
            ...
            if (gestion.valido === 'true' && nivelAplicacion.valido === 'true') {
                axios.post(URL + '/reportes3/listartodosvariable-mun', { id: gestion.campo, }).then(json => {
                    if (json.data.ok) {
                        setListaGrupo(json.data.data)
                        // console.log(json.data.data,'data de la bd')
                    } else alert2({ icono: 'warning', titulo: 'Intente Nuevamente mas tarde!', boton: 'ok', texto: json.data.msg })
                ...
                <div className="container_reportes_formulario">
                    <div className='header-reportes'>
                        ...
                        <p style={{ fontSize: '15px', fontWeight: 'bold', color: '#595959', textAlign: 'center', marginBottom: '0' }}
                            className='text-center'>{localStorage.getItem('red')}  </p>
                    ...
                    <div className='cajaprimario-reportes m-0 m-sm-2 m-md-3 m-lg-3 mt-0 mt-lg-0 '>
                        <div className='contenedor cajareportes p-3 p-sm-3 p-md-4 p-lg-4  pt-2 pb-2'>
                            {/* {
                                alc.campo == 1 && <>
                                    <div className='row'>
                                        ...
                                        <div className='col-5'>
                                            {listaSs.map(e => (
                                                ss.campo == e.id && <p className='titulo-reportes' style={{ color: '#2980B9', textAlign: 'left', paddingLeft: '8px', marginBottom: '0', }}>
                                                    {'SUB-SECTOR: ' + e.nombre} </p>
                                            ))}
                                        </div>
                                    </div>
                                    <div className='row'>
                            ...
                            {/* {alc.campo == 2 && <p className='titulo-reportes' style={{ color: '#2980B9', textAlign: 'left', paddingLeft: '8px', marginBottom: '0', }}>
                                                ...
                                                <>
                                                    {cabecera.length > 0 &&
                                                        <thead className='cab-form'>
                                                            <tr  >
                                                                <th className="col-3 mincelda var" style={{ color: '#595959', background: 'AliceBlue', borderRight: '1px solid #000040', borderTop: '1px solid #000040', }}>ESTABLECIMIENTO</th>
                                            ...
                                            {alc.campo == 2 &&
                                                <>
                                                    {cabecera.length > 0 &&
                                                        <thead className='cab-form'>
                                                            <tr  >
                                                                <th className="col-3 mincelda var" style={{ color: '#595959', background: 'AliceBlue', borderRight: '1px solid #000040', borderTop: '1px solid #000040', }}>MUNICIPIO</th>
                                                                <th className="col-3 mincelda var" style={{ color: '#595959', background: 'AliceBlue', borderRight: '1px solid #000040', borderTop: '1px solid #000040', }}>VARIABLE</th>
                                                                {cabecera.map(cb => (
                                                                    parseInt(cb.nivel) == 1 &&
                                                                    <th className='text-center nivel1F' style={{
...
export default ReportesFormularioRed;
