...
function Mireportes2() {
    ...
    try {
        ...
        const procesar = async () => {
            ...
            if (mes1.valido === 'true' && mes2.valido === 'true' && gestion.valido === 'true' && ss.valido === 'true') {
                setEstado(1)
                setTexto('Procesando la informacion...')
                if (variablesSeleccionado.length > 0) {
                    axios.post(URL + '/reportes2/reportes-formularios-dividido-sedes', { lista: variablesSeleccionado, variable: grupoSeleccionados[0], gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                        if (json.data.ok) {
                    ...
                    axios.post(URL + '/reportes2/reportes-formularios-enteros-sedes', { lista: grupoSeleccionados, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                        if (json.data.ok) {
                            console.log('datos del server', json.data)
                            setData(json.data.dataForm)
                            setCabecera(json.data.cabeceras)
        ...
        const excel = () => {
            ...
            principal.getCell('C5').value = 'NIVEL FORMULARIO: SEDES'
            ...
            workbook.xlsx.writeBuffer().then(data => {
                ...
                anchor.href = url;
                anchor.download = 'FORMULARIO ADICIONAL 301C ' + gestion_ + '_' + mes1_ + '-' + mes2_ + '-SEDES CH.xlsx';
                    ...
                    <div className="container_reportes m-auto" style={{ width: '97%' }}>
                        ...
                        <div className='p-2'>
                            ...
                            <div className='botonModal p-1'>
                                <button className='botonVolversm' onClick={() => window.location.href = '/mireportes2'}>Volver</button>
