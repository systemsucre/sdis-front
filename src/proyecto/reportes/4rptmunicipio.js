            ...
            if (mes1.valido === 'true' && mes2.valido === 'true' && gestion.valido === 'true' && estab.valido === 'true')
                if (estab.campo === 1000) {
                    ...
                    if (variablesSeleccionado.length > 0) {
                        axios.post(URL + '/reportes4/reportes-formularios-dividido-est-nivel-4', { lista: variablesSeleccionado, variable: grupoSeleccionados[0], gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                        ...
                        axios.post(URL + '/reportes4/reportes-formularios-enteros-est-nivel-4', { lista: grupoSeleccionados, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                                console.log('datos del server', json.data)
                                setData(json.data.dataForm)
                                setCabecera(json.data.cabeceras)
                    ...
                    setTexto('Procesando la informacion...')
                    if (variablesSeleccionado.length > 0) {
                        axios.post(URL + '/reportes4/reportes-formularios-dividido-est-nivel-5', { lista: variablesSeleccionado, variable: grupoSeleccionados[0], gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo, est: estab.campo }).then(json => {
                            if (json.data.ok) {
                                console.log('datos del server', json.data)
                                setData(json.data.dataForm)
                                setCabecera(json.data.cabeceras)
                        ...
                        axios.post(URL + '/reportes4/reportes-formularios-enteros-est-nivel-5', { lista: grupoSeleccionados, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo, est: estab.campo }).then(json => {
                            if (json.data.ok) {
                                console.log('datos del server', json.data)
                                setData(json.data.dataForm)
                                setCabecera(json.data.cabeceras)
                            ...
                            {listaGrupo.map(lg => (
                                grupoSeleccionados.map(gs => (
                                    parseInt(lg.id) === parseInt(gs) && <div key={gs}>
                                        <p style={{ fontSize: '10px', marginBottom: '0', fontWeight: 'bold' }}>{lg.nombre}  </p>
