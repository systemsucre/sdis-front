...
function ReportesFormularioSedes() {
    const auth = useAuth()
    ...
    const [alcanceEst, setAlcalceEst] = useState([{ id: 4, nombre: 'SEDES' }, { id: 3, nombre: 'RED' }, { id: 2, nombre: 'MUNICIPIO' }, { id: 1, nombre: 'ESTABLECIMIENTO' }]);
    const [alcanceMunicipio, setAlcalceMunicipio] = useState([{ id: 4, nombre: 'SEDES' }, { id: 3, nombre: 'RED' }, { id: 2, nombre: 'MUNICIPIO' }]);
    const [alcanceRed, setAlcalceRed] = useState([{ id: 4, nombre: 'SEDES' }, { id: 3, nombre: 'RED' }]);

    const [alcance, setAlcalce] = useState([{ id: 4, nombre: 'SEDES' }, { id: 3, nombre: 'RED' }, { id: 2, nombre: 'MUNICIPIO' }, { id: 1, nombre: 'ESTABLECIMIENTO' }]);

    const [listaNivelAplicacion, setListaNivelAplicacion] = useState([{ id: 1, nombre: 'ESTABLECIMIENTOS' }, { id: 2, nombre: 'MUNICIPIO' }, { id: 3, nombre: 'RED' }]);
    ...
    const [alc, setAlc] = useState({ campo: 3, valido: 'true' })
    ...
    useEffect(() => {
        ...
        if (nivelAplicacion.campo == 1) {
            setAlcalce([])
            alcanceEst.forEach(e => {
                data.push(e)
            })
        }
        if (nivelAplicacion.campo == 2) {
            alcanceMunicipio.forEach(e => {
                data.push(e)
            })
        }
        if (nivelAplicacion.campo == 3) {
            alcanceRed.forEach(e => {
                data.push(e)
            })
        }
    ...
    try {
        ...
        const listarTodosGrupos = async (id = null) => {
            console.log('todos los grupos est')
            if (gestion.valido === 'true' || id) {
        ...
        const listarGrupos = async () => {
            console.log('grupos elegidos est')
            if (gestion.valido === 'true' && nivelAplicacion.valido === 'true') {
        ...
        const listarTodosGruposMun = async () => {
            console.log('todos los grupos mun')
            if (gestion.valido === 'true' && nivelAplicacion.valido === 'true') {
                axios.post(URL + '/reportes2/listartodosvariable-mun', { id: gestion.campo, }).then(json => {
                    if (json.data.ok) {
                        setListaGrupo(json.data.data)
                        // console.log(json.data.data,'data de la bd')
                    } else alert2({ icono: 'warning', titulo: 'Intente Nuevamente mas tarde!', boton: 'ok', texto: json.data.msg })
        ...
        const listarGruposMun = async () => {
            console.log('grupos elegidos mun')
            if (gestion.valido === 'true' && nivelAplicacion.valido === 'true') {
        ...
        const listarTodosGruposRed = async () => {
            console.log('todos los grupos mun')
            if (gestion.valido === 'true' && nivelAplicacion.valido === 'true') {
                axios.post(URL + '/reportes2/listartodosvariable-red', { id: gestion.campo, }).then(json => {
                    if (json.data.ok) {
                        setListaGrupo(json.data.data)
                        // console.log(json.data.data,'data de la bd')
                    } else alert2({ icono: 'warning', titulo: 'Intente Nuevamente mas tarde!', boton: 'ok', texto: json.data.msg })
        ...
        const procesar = async () => {
            if (mes1.valido === 'true' && mes2.valido === 'true' && gestion.valido === 'true' && idVariable.valido === 'true' && alc.valido === 'true') {
                ...
                if (nivelAplicacion.campo == 1) {
                    if (alc.campo == 1) {
                        axios.post(URL + '/reportes2/procesar-por-establecimiento', { variable: idVariable.campo, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                    ...
                    if (alc.campo == 2) {
                        ...
                        axios.post(URL + '/reportes2/procesar-por-municipio', { variable: idVariable.campo, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                                setListaEstablecimientos(json.data.conf)
                                setData(json.data.data)
                                setCabecera(json.data.cabecera)
                    ...
                    if (alc.campo == 3) {
                        axios.post(URL + '/reportes2/procesar-por-red', { variable: idVariable.campo, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                                setListaEstablecimientos(json.data.conf)
                                setData(json.data.data)
                                setCabecera(json.data.cabecera)
                    ...
                    if (alc.campo == 4) {
                        axios.post(URL + '/reportes2/procesar-est-consolidado', { variable: idVariable.campo, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                                setListaEstablecimientos(json.data.conf)
                                setData(json.data.data)
                                setCabecera(json.data.cabecera)
                ...
                if (nivelAplicacion.campo == 2) {
                    if (alc.campo == 2) {
                        axios.post(URL + '/reportes2/procesar-por-variable-municipio-municipio', { variable: idVariable.campo, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                                setListaEstablecimientos(json.data.conf)
                                setData(json.data.data)
                                setCabecera(json.data.cabecera)
                    ...
                    if (alc.campo == 3) {
                        axios.post(URL + '/reportes2/procesar-por-variable-municipio-red', { variable: idVariable.campo, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                                setListaEstablecimientos(json.data.conf)
                                setData(json.data.data)
                                setCabecera(json.data.cabecera)
                    ...
                    if (alc.campo == 4) {
                        axios.post(URL + '/reportes2/procesar-por-variable-municipio-consolidado', { variable: idVariable.campo, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                                setListaEstablecimientos(json.data.conf)
                                setData(json.data.data)
                                setCabecera(json.data.cabecera)
                ...
                if (nivelAplicacion.campo == 3) {
                    if (alc.campo == 3) {
                        axios.post(URL + '/reportes2/procesar-por-variable-red-red', { variable: idVariable.campo, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                                setListaEstablecimientos(json.data.conf)
                                setData(json.data.data)
                                setCabecera(json.data.cabecera)
                    ...
                    if (alc.campo == 4) {
                        axios.post(URL + '/reportes2/procesar-por-variable-red-consolidado', { variable: idVariable.campo, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok) {
                                setListaEstablecimientos(json.data.conf)
                                setData(json.data.data)
                                setCabecera(json.data.cabecera)
            ...
            } else alert2({ icono: 'question', titulo: 'selecione los parametros año, mes1, mes2, formulario y consolidacion de reporte', boton: 'ok' })
        ...
        const excel = () => {
            ...
            principal.getCell('C4').value = 'SEDES CH'
            ...
            <div style={{ background: '#e5e5e5', paddingTop: '0.5rem', paddingBottom: '0.5rem', minHeight: '90.5vh' }} >
                ...
                <div className="container_reportes_formulario">
                    <div className='header-reportes'>
                        ...
                        <p style={{ fontSize: '15px', fontWeight: 'bold', color: '#595959', textAlign: 'center', marginBottom: '0' }}
                            className='text-center'>SEDES CH </p>
                    ...
                    <div className='p-0 p-sm-2 p-md-3 p-lg-3 pb-0 pt-0 pt-lg-0 pt-md-0 pb-lg-0'>
                        <div className='orden-tiempo '>
                            <div className='col-12'>
                                <div className='row '>
                                    <div className='col-4 col-sm-4 col-md-4 col-lg-2 p-1' onClick={() => {
                                        setAlc({ campo: null, valido: null });
                                        setListaGrupo([]); setIdVariable({ campo: null, valido: null }); setSs({ campo: null, valido: null }); setCabecera([])
                                    }} >
                                        <SelectSM
                            ...
                            {cabecera.length > 0 && <>
                                {
                                    listaGrupo.map(e => (
                                        ...
                                        <p style={{ fontSize: '13px', marginBottom: '0', fontWeight: 'bold' }} >{e.nombre} </p>
                                    ...
                                    <Table className=' table-sm' style={{ border: "1px solid #dee2e6", borderRight: 'none', borderTop: '1px solid white', borderSpacing: '0px', padding: '0px' }} >
                                        {cabecera.length > 0 &&
                                            <thead className='cab-form'>
                                                <tr  >
                                                    <th className="col-3 mincelda var" style={{ color: '#595959', background: 'AliceBlue', borderRight: '1px solid #dee2e6', borderTop: '1px solid #dee2e6', }}>ENTIDAD</th>
                                        ...
                                        <tbody>
                                            {
                                                listaEstablecimientos.map((e, index) => (
                                                    ...
                                                    <tr className='item' key={index} style={{ borderRight: '1px solid #dee2e6', background: e.color == 1 ? '#FDEDEC' : null, }}>
                                                        <td className="col-3 mincelda titulo-indicador"
                                                            style={{
                                                        ...
                                                        {
                                                            data.map(d => (
                                                                e.idest == d.idest && parseInt(d.indicador) === parseInt(e.id) &&
...
export default ReportesFormularioSedes;
