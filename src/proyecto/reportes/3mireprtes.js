...
function Mireportes3() {
    ...
    try {
        ...
        const procesar = async () => {
            ...
            if (mes1.valido === 'true' && mes2.valido === 'true' && gestion.valido === 'true') {
                ...
                if (variablesSeleccionado.length > 0) {
                    axios.post(URL + '/reportes3/reportes-formularios-dividido-red', { lista: variablesSeleccionado, variable: grupoSeleccionados[0], gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                        if (json.data.ok) {
                    ...
                    axios.post(URL + '/reportes3/reportes-formularios-enteros-red', { lista: grupoSeleccionados, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                        if (json.data.ok) {
                            console.log('datos del server', json.data)
                            setData(json.data.dataForm)
                            setCabecera(json.data.cabeceras)
        ...
        const excel = () => {
            ...
            principal.getCell('A6').value = localStorage.getItem('red')
            ...
            workbook.xlsx.writeBuffer().then(data => {
                ...
                anchor.href = url;
                anchor.download = 'FORMULARIO ADICIONAL 301C ' + gestion_ + '_' + mes1_ + '-' + mes2_ + '-' + localStorage.getItem('red') + '.xlsx';
