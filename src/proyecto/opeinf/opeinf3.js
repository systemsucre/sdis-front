    ...
    try {
        ...
        useEffect(() => {
            ...
            if (listaGestion.length < 1) {
                document.title = 'OPORTUNIDAD DE INFORMACION RED'
                listarGestion()
                listarGrupoInicio()
                listarMun()
            }
        ...
        const listarGestion = async () => {
            axios.post(URL + '/opeinf3/listargestion').then(json => {
        ...
        const listarMes = async () => {
            if (gestion.valido === 'true') {
                axios.post(URL + '/opeinf3/listarmes', { id: gestion.campo, fecha: fecha + ' ' + horafinal }).then(json => {
                    if (json.data.ok) {
        ...
        const listarMun = async () => {
            axios.post(URL + '/opeinf3/listarmunicipio').then(json => {
                if (json.data.ok) {
        ...
        const listarGrupoInicio = async () => {
            axios.post(URL + '/opeinf3/listarvariableinicio').then(json => {
                if (json.data.ok) {
        ...
        const listarGrupos = async () => {
            if (gestion.valido === 'true') {
                axios.post(URL + '/opeinf3/listarvariable', { id: gestion.campo }).then(json => {
                    if (json.data.ok) {
                        setListaGrupo(json.data.data)
        ...
        const listarVariables = async (id) => {
            ...
            setEstado(1)
            setTexto('Cargando...')
            axios.post(URL + '/opeinf3/listarindicadores', { variable: id }).then(json => {
                if (json.data.ok) {
        ...
        const procesar = async () => {
            ...
            if (mes1.valido === 'true' && mes2.valido === 'true' && gestion.valido === 'true' && municipio.valido === 'true')
                if (grupoSeleccionados.length > 0) {
                    ...
                    grupoSeleccionados.forEach(e => {
                        axios.post(URL + '/opeinf3/listarcabeceras', { variable: e }).then(json => {
                            if (json.data.ok)
                        ...
                        axios.post(URL + '/opeinf3/listarindicadores', { variable: e }).then(json => {
                            if (json.data.ok)
                                json.data.data.forEach(e => {
                                    dataInd_.push(e)
                                })
                        ...
                        axios.post(URL + '/opeinf3/all-inf', { municipio: municipio.campo, variable: e, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok)
                                json.data.data.forEach(e => {
                                    data_.push(e)
                                })
            ...
            <div style={{ background: '#e5e5e5', paddingTop: '0.5rem', paddingBottom: '0.5rem', height: '95vh' }} >
                ...
                {ventana === 0 ?
                    <div className="container_reportes">
                        ...
                        <p className='texto-mov' style={{ marginBottom: '0', fontSize: '12px' }}>{'RED :: ' + localStorage.getItem('red')}</p>
                    ...
                    <div className="container_reportes m-auto" style={{ width: '97%' }}>
                        ...
                        {listaMunicipio.map(e => (
                            parseInt(e.id) === parseInt(municipio.campo) &&
                            <p key={e.id} className='titulo-reportes' style={{ textAlign: 'left', paddingLeft: '8px', marginBottom: '0', fontSize: '11px' }}>MUNICIPIO : {e.nombre}</p>
                        ))}
                        ...
                        <div className='p-2'>
                            ...
                            <div className='botonModal p-1'>
                                <button className='botonProcesar' onClick={() => window.location.href = '/oportunidad-de-informacion-redes'}>CERRAR</button>
                            </div>
                        </div>
                    </div>
                }
