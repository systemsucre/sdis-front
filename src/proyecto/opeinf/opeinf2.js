    ...
    try {
        ...
        const listarMes = async () => {
            if (gestion.valido === 'true') {
                axios.post(URL + '/opeinf2/listarmes', { id: gestion.campo, fecha: fecha + ' ' + horafinal }).then(json => {
                    if (json.data.ok) {
        ...
        const listarRed = async () => {
            axios.post(URL + '/opeinf2/listarred').then(json => {
                if (json.data.ok) {
                    setListaRed(json.data.data)
        ...
        const listarGrupoInicio = async () => {
            axios.post(URL + '/opeinf2/listarvariableinicio').then(json => {
                if (json.data.ok) {
        ...
        const listarGrupos = async () => {
            if (gestion.valido === 'true') {
                axios.post(URL + '/opeinf2/listarvariable', { id: gestion.campo }).then(json => {
                    if (json.data.ok) {
                        setListaGrupo(json.data.data)
        ...
        const listarVariables = async (id) => {
            ...
            setEstado(1)
            setTexto('Cargando...')
            axios.post(URL + '/opeinf2/listarindicadores', { variable: id }).then(json => {
                if (json.data.ok) {
        ...
        const procesar = async () => {
            ...
            if (mes1.valido === 'true' && mes2.valido === 'true' && gestion.valido === 'true' && red.valido === 'true')
                if (grupoSeleccionados.length > 0) {
                    ...
                    grupoSeleccionados.forEach(e => {
                        axios.post(URL + '/opeinf2/listarcabeceras', { variable: e }).then(json => {
                            if (json.data.ok)
                        ...
                        axios.post(URL + '/opeinf2/listarindicadores', { variable: e }).then(json => {
                            if (json.data.ok)
                                json.data.data.forEach(e => {
                                    dataInd_.push(e)
                                })
                        ...
                        axios.post(URL + '/opeinf2/all-inf', { red: red.campo, variable: e, gestion: gestion.campo, mes1: mes1.campo, mes2: mes2.campo }).then(json => {
                            if (json.data.ok)
                                json.data.data.forEach(e => {
                                    data_.push(e)
                                })
            ...
            else alert2({ icono: 'question', titulo: 'selecione los parametros año, mes1, mes2 y red', boton: 'ok' })
            ...
            <div style={{ background: '#e5e5e5', paddingTop: '0.5rem', paddingBottom: '0.5rem', height: '95vh' }} >
                ...
                {ventana === 0 ?
                    <div className="container_reportes">
                        ...
                        <p className='texto-mov' style={{ marginBottom: '0', fontSize: '12px' }}>SEDES CHUQUISACA</p>
                    ...
                    <div className="container_reportes m-auto" style={{ width: '97%' }}>
                        ...
                        {listaRed.map(e => (
                            parseInt(e.id) === parseInt(red.campo) &&
                            <p key={e.id} className='titulo-reportes' style={{ textAlign: 'left', paddingLeft: '8px', marginBottom: '0', fontSize: '11px' }}>RED : {e.nombre}</p>
                        ))}
                        ...
                        <div className='p-2'>
                            ...
                            <div className='botonModal p-1'>
                                <button className='botonProcesar' onClick={() => window.location.href = '/oportunidad-de-informacion-sedes'}>CERRAR</button>
                            </div>
                        </div>
                    </div>
                }
