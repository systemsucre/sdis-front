...
function Formulario6() {
    ...
    try {
        ...
        const listarGestion = async () => {
            axios.post(URL + '/formulario6/listargestion').then(json => {
        ...
        const listarMes = async () => {
            if (gestion.valido === 'true') {
                axios.post(URL + '/formulario6/listarmes', { id: gestion.campo, fecha: fecha + ' ' + horafinal }).then(json => {
                    if (json.data.ok) {
        ...
        const listarVariable = async () => {
            if (gestion.valido === 'true') {
                if (mes.valido === 'true') {
                    if (ss.valido === 'true') {
                        axios.post(URL + '/formulario6/listarvariable', { id: gestion.campo, ss: ss.campo }).then(json => {
                            if (json.data.ok) {
            ...
            if (gestion.valido === 'true') {
                if (mes.valido === 'true') {
                    if (variable.valido === 'true') {
                        setEstado(1)
                        setTexto('Cargando...')
                        axios.post(URL + '/formulario6/listarindicadores', { variable: variable.campo, fecha: fecha, mes: mes.campo }).then(json => {
                            if (json.data.ok) {
                                ...
                                if (cabeceras) {
                                    axios.post(URL + '/formulario6/listarcabeceras', { variable: variable.campo }).then(json => {
                                        if (json.data.ok) {
        ...
        const guardar = async () => {
            if (mes.valido === 'true' && variable.valido === 'true' && datosVariable.length > 0 && estado === 0 && gestion.valido === 'true' && estadomes) {
                ...
                if (accion.isConfirmed) {
                    ...
                    axios.post(URL + '/formulario6/guardar', {
                        mes: mes.campo,
        ...
        return (
            <div className="container_formulario">
                <div className='contenedor-cabecera row' >
                    ...
                    <p style={{ fontSize: '16px', fontWeight: 'bold', marginBottom: '0', color:'#595959'}} className='text-center'>USUARIO AREA</p>
                </div>
